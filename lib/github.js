...
function getToken(user) {
  ...
  // The column is encrypted at rest; a row written before that is passed
  // through unchanged, and one that cannot be decrypted yields null instead of
  // a bearer token GitHub would only reject.
  if (user && user.github_token) return secretBox.read(user.github_token);
  return null;
}
...
module.exports = {
  initGithub,
  getToken,
  // Test hooks: pure/near-pure internals that would otherwise only be
  // reachable through a network call.
  _cachedFetch: cachedFetch,
  _OS_MULTIPLIERS: OS_MULTIPLIERS,
