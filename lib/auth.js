...
const {
  createUser, findUserByApiKey, findUserById, findDeviceByApiKey,
  updateLastLogin, updateUserGithubToken, createSession, getSession, deleteSession
} = require('./db');
...
function authenticateApiKey(req) {
  ...
  const device = findDeviceByApiKey(match[1]);
  if (!device) {
    // Fallback: legacy lookup in users table
    if (!MULTI_USER) return { user: DUMMY_USER, device: null };
    const user = findUserByApiKey(match[1]);
    return user ? { user, device: null } : null;
  }

  if (!MULTI_USER) return { user: DUMMY_USER, device };
  const user = findUserById(device.user_id);
  return user ? { user, device } : null;
}
  ...
  if (pathname === '/auth/github/callback' && req.method === 'GET') {
    ...
    httpsRequest({
      ...
      return httpsRequest({
        ...
        updateUserGithubToken(user.id, accessToken);
