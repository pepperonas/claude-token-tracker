...
function buildUserSnapshot(db, { userId, multiUser, dir } = {}) {
  ...
  // One transaction around the whole copy, so every table is read from the
  // same point in time. Without it a second writer on the database (a restore
  // script, a maintenance session, a second instance) can commit a message
  // between the `messages` copy and the `message_tools` copy — the tool rows
  // then reference a message the snapshot does not have and the foreign key
  // fails mid-download. ATTACH itself cannot run inside a transaction, so it
  // stays outside.
