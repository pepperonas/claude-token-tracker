...
function insertMessagesForUser(messages, costFn, userId) {
  const insertMsg = db.prepare(`
    INSERT OR REPLACE INTO messages
      (id, timestamp, model, session_id, project, input_tokens, output_tokens,
       cache_read_tokens, cache_create_tokens, stop_reason, cost,
       lines_added, lines_removed, lines_written, user_id, is_subagent)
    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `);
...
function insertRateLimitEventsForUser(events, userId) {
  if (!events || events.length === 0) return;
  const insert = db.prepare(
    'INSERT OR IGNORE INTO rate_limit_events (id, timestamp, session_id, project, user_id) VALUES (?, ?, ?, ?, ?)'
  );
  const insertAll = db.transaction((evts) => {
    for (const evt of evts) {
      insert.run(evt.id, evt.timestamp, evt.sessionId || null, evt.project || null, userId);
    }
  });
  insertAll(events);
}
...
function getRateLimitEventsForUser(userId) {
  return db.prepare('SELECT * FROM rate_limit_events WHERE user_id = ? ORDER BY timestamp').all(userId).map(row => ({
    id: row.id,
    timestamp: row.timestamp,
    sessionId: row.session_id,
    project: row.project
