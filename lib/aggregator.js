  ...
  getSessions(project, model, from, to) {
    return Object.entries(this._sessions)
      .filter(([_id, s]) => {
        ...
        if (from && s.lastTs.slice(0, 10) < from) return false;
        if (to && s.firstTs.slice(0, 10) > to) return false;
        return true;
      })
