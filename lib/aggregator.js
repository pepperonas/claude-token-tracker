...
class Aggregator {
  ...
  getProjectDetail(name, from, to) {
    ...
    for (const msg of this.messages) {
      ...
      // Daily
      if (!daily[date]) daily[date] = { inputTokens: 0, outputTokens: 0, cacheReadTokens: 0, cacheCreateTokens: 0, cost: 0, messages: 0 };
      const dd = daily[date];
    ...
    return {
      ...
      firstTs, lastTs, totalDurationMin,
...
class AggregatorCache {
  constructor(getMessagesForUser, getRateLimitEventsForUser) {
    this._cache = new Map(); // "userId:deviceId" -> { aggregator, lastAccess }
  ...
  get(userId, deviceId) {
    ...
    if (!entry) {
      ...
      entry = { aggregator: agg, lastAccess: Date.now() };
