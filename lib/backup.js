...
const { getDB, getAllMessages, getMessagesForUser } = require('./db');
...
function backupToPath(destDir) {
  ...
  // The name is second-resolution, so two backups within the same second would
  // collide — VACUUM INTO then fails with a bare "output file already exists".
  let destFile = path.join(destDir, `tracker-backup-${timestamp}.db`);
  for (let n = 2; fs.existsSync(destFile); n++) {
    destFile = path.join(destDir, `tracker-backup-${timestamp}-${n}.db`);
  }
  ...
  // Reject backups smaller than 50% of the last one (prevents saving corrupt/empty DBs)
  const existing = fs.readdirSync(destDir)
    .filter(f => f.startsWith('tracker-backup-') && f.endsWith('.db') && f !== path.basename(destFile))
    .sort()
    .reverse();
  if (existing.length > 0) {
    const lastSize = fs.statSync(path.join(destDir, existing[0])).size;
    if (newSize < lastSize * 0.5) {
      fs.unlinkSync(destFile);
      throw new Error(`Backup rejected: ${newSize} bytes is <50% of last backup (${lastSize} bytes)`);
    }
  }

  return { path: destFile, timestamp, size: newSize };
}
 ...
 *
 * `userId` selects one account's messages. It is omitted only on a
 * single-user instance, where every row belongs to the one local account
 * anyway.
 */
function exportJSON(userId = null) {
  const messages = userId == null ? getAllMessages() : getMessagesForUser(userId);
  return {
