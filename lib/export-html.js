...
/* Responsive */
@media(max-width:768px){.kpi-grid{grid-template-columns:repeat(2,1fr)}.chart-grid{grid-template-columns:1fr}}
...
<div class="tab-panel" id="tab-projects">
  <div class="chart-grid full"><div class="chart-box"><h3>Tokens by Project</h3><div class="chart-container" style="height:${Math.max(200, projects.length * 28)}px"><canvas id="c-projects"></canvas></div></div></div>
...
<div class="tab-panel" id="tab-tools">
  <div class="chart-grid full"><div class="chart-box"><h3>Tool Usage</h3><div class="chart-container" style="height:${Math.max(200, (tools || []).length * 24)}px"><canvas id="c-tools"></canvas></div></div></div>
...
function renderOverview(){
  ...
  ]},options:{responsive:true,maintainAspectRatio:false,plugins:{tooltip:{callbacks:{label:function(c){return c.dataset.label+': '+fmt(c.raw)}}}},scales:{x:{stacked:true,grid:{display:false}},y:{stacked:true,ticks:{callback:function(v){return fmt(v)}}}}}});
  new Chart(document.getElementById('c-daily-cost'),{type:'line',data:{labels:d.map(function(x){return fmtD(x.date)}),datasets:[{label:'Cost',data:d.map(function(x){return x.cost}),borderColor:C.cost,backgroundColor:'rgba(57,210,192,0.1)',fill:true,tension:0.3,pointRadius:1}]},options:{responsive:true,maintainAspectRatio:false,scales:{x:{grid:{display:false}},y:{ticks:{callback:function(v){return fmtC(v)}}}}}});
  var m=DATA.models;
  new Chart(document.getElementById('c-model-dist'),{type:'doughnut',data:{labels:m.map(function(x){return x.label}),datasets:[{data:m.map(function(x){return x.inputTokens+x.outputTokens+x.cacheReadTokens+x.cacheCreateTokens}),backgroundColor:C.models.slice(0,m.length)}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{position:'right'}}}});
  var h=DATA.hourly;
  if(h.length){new Chart(document.getElementById('c-hourly'),{type:'bar',data:{labels:h.map(function(x){return String(x.hour).padStart(2,'0')+':00'}),datasets:[{label:'Messages',data:h.map(function(x){return x.messages}),backgroundColor:C.input}]},options:{responsive:true,maintainAspectRatio:false,scales:{x:{grid:{display:false}},y:{beginAtZero:true}}}})}
}
...
function renderCharts(){
  ...
  ]},options:{responsive:true,maintainAspectRatio:false,scales:{x:{stacked:true,grid:{display:false}},y:{stacked:true,ticks:{callback:function(v){return fmtC(v)}}}}}});
  var cum=0;var cumD=d.map(function(x){cum+=x.cost||0;return{date:x.date,cost:Math.round(cum*100)/100}});
  new Chart(document.getElementById('c-cumulative-cost'),{type:'line',data:{labels:cumD.map(function(x){return fmtD(x.date)}),datasets:[{label:'Cumulative',data:cumD.map(function(x){return x.cost}),borderColor:C.cost,backgroundColor:'rgba(57,210,192,0.08)',fill:true,tension:0.3,pointRadius:1}]},options:{responsive:true,maintainAspectRatio:false,scales:{x:{grid:{display:false}},y:{ticks:{callback:function(v){return fmtC(v)}}}}}});
  var w=DATA.weekday;
  if(w.length){new Chart(document.getElementById('c-weekday'),{type:'bar',data:{labels:w.map(function(x){return x.day}),datasets:[{label:'Messages',data:w.map(function(x){return x.messages}),backgroundColor:C.input},{label:'Cost',data:w.map(function(x){return x.cost}),backgroundColor:C.cost,yAxisID:'y1'}]},options:{responsive:true,maintainAspectRatio:false,scales:{x:{grid:{display:false}},y:{position:'left',beginAtZero:true},y1:{position:'right',grid:{drawOnChartArea:false},ticks:{callback:function(v){return fmtC(v)}}}}}})}
  var sr=DATA.stopReasons;
  if(sr.length){new Chart(document.getElementById('c-stop-reasons'),{type:'doughnut',data:{labels:sr.map(function(x){return x.reason}),datasets:[{data:sr.map(function(x){return x.count}),backgroundColor:C.models.slice(0,sr.length)}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{position:'right'}}}})}
...
function renderProjects(){
  var p=DATA.projects;
  new Chart(document.getElementById('c-projects'),{type:'bar',data:{labels:p.map(function(x){return x.name}),datasets:[
  ...
  ]},options:{indexAxis:'y',responsive:true,maintainAspectRatio:false,scales:{x:{stacked:true,ticks:{callback:function(v){return fmt(v)}}},y:{stacked:true}}}});
...
function renderTools(){
  var t=DATA.tools;
  if(t.length){new Chart(document.getElementById('c-tools'),{type:'bar',data:{labels:t.map(function(x){return x.name}),datasets:[{label:'Calls',data:t.map(function(x){return x.count}),backgroundColor:C.input}]},options:{indexAxis:'y',responsive:true,maintainAspectRatio:false,scales:{x:{ticks:{callback:function(v){return fmtN(v)}}}}}})}
...
function renderAchievements(){
  ...
  if(tData.length){
    new Chart(document.getElementById('c-achievements'),{type:'bar',data:{labels:tData.map(function(x){return fmtD(x.date)}),datasets:[
      ...
      {label:'Cumulative Points',data:tData.map(function(x){return x.cumPts}),borderColor:C.cost,backgroundColor:'transparent',type:'line',yAxisID:'y1',tension:0.3,pointRadius:2,order:1}
    ]},options:{responsive:true,maintainAspectRatio:false,scales:{x:{grid:{display:false}},y:{position:'left',beginAtZero:true,ticks:{stepSize:1}},y1:{position:'right',beginAtZero:true,grid:{drawOnChartArea:false}}}}});
  }
